...
function getSavings(totalBudget) {
  ...
  limitElm.forEach((elm) => {
    ...
    totalCategoryAmount.push(limitAmount);
  });
  ...
  savingsElm.textContent =
    totalBudget - totalCategoryAmount.reduce((a, b) => a + b, 0);
}
...
function getTotalExpense() {
  ...
  // set the savings element textContent to the totalBudget (which we will get from
  // the checkBudget and the addBudget functions) + the sum of the totalCategoryAmount array
  totalExpenseElm.textContent = totalCategoryAmount.reduce((a, b) => a + b, 0);
}
